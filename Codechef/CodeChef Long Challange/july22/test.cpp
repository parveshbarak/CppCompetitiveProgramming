...
bool merge(string s, int low, int mid, int high) {
    int k = mid;
    while(low<mid) {
        if(s[low++] != s[k++]) return false;
    }
    return true;
}

bool mergeSort(string s, int low, int high) {
    if(low >= high) return true;
    if((low+high)%2==0) high -= 1;
    int mid = (low+high)/2;
    return mergeSort(s, low, mid) &&  merge(s, low, mid+1, high);
}
