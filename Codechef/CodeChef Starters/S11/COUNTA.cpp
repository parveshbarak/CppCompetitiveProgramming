...
void code_here() {
    ...
    while(t--) {
        ...
        ll f,ct=0; bool fall = false;
        if(n==2) cout << (2*100000 - (2*b[0]) + 1) << "\n";
        else {
            if(b[0]>b[1]) { ll ans = 2*100000-(2*b[0]+1); cout << ans << "\n"; }
            else {
                loop(i,0,n-2) {
                    if(b[i+1]>b[i]) fall = true, f = i, ct++;
                }
                if(!fall) { ll ans = 2*100000-(2*b[n-2])+1; cout << ans*100 << "\n"; }
                else { ll ans = 2*100000-(2*b[f])+1; cout << ans << "\n"; }
            }
        }
